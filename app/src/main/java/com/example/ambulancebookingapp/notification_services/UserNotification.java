package com.example.ambulancebookingapp.notification_services;
...
import com.example.ambulancebookingapp.UserSideScreens.PatientDashboardActivity;
import com.example.ambulancebookingapp.models.NotificationModel;
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                for(DocumentSnapshot snapshot : task.getResult())
                {
                    if(snapshot.get("to").toString().equals(firebaseAuth.getCurrentUser().getUid()) &&
                            Integer.parseInt(snapshot.get("isRead").toString()) == 1)
                    {
                        notificationID++;
